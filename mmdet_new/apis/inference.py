...
from mmdet_new.core import get_classes
...
def init_detector(config, checkpoint=None, device='cuda:0', cfg_options=None):
    ...
    Args:
        config (str, :obj:`Path`, or :obj:`mmcv_new.Config`): Config file path,
    ...
    if isinstance(config, (str, Path)):
        config = mmcv_new.Config.fromfile(config)
    elif not isinstance(config, mmcv_new.Config):
...
class LoadImage:
    ...
    def __call__(self, results):
        ...
        warnings.warn('`LoadImage` is deprecated and will be removed in '
                      ...
                      'from `mmdet_new.datasets.pipelines.` instead.')
        ...
        img = mmcv_new.imread(results['img'])
        results['img'] = img
