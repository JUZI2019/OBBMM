...
import mmcv_new
import torch
from mmcv_new.utils import TORCH_VERSION, digit_version, print_log
...
def update_data_root(cfg, logger=None):
    ...
    Args:
        cfg (mmcv_new.Config): The model config need to modify
        logger (logging.Logger | str | None): the way to print msg
    """
    assert isinstance(cfg, mmcv_new.Config), \
        f'cfg got wrong type: {type(cfg)}, expected mmcv_new.Config'
    ...
    def update(cfg, src_str, dst_str):
        for k, v in cfg.items():
            if isinstance(v, mmcv_new.ConfigDict):
