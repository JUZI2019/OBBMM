...
def collect_annotations(files, nproc=1):
    ...
    if nproc > 1:
        images = mmcv_new.track_parallel_progress(
            load_img_info, files, nproc=nproc)
    else:
        images = mmcv_new.track_progress(load_img_info, files)
...
def load_img_info(files):
    ...
    inst_img = mmcv_new.imread(inst_file, 'unchanged')
...
def cvt_annotations(image_infos, out_json_name):
    ...
    mmcv_new.dump(out_json, out_json_name)
    return out_json
...
def main():
    ...
    mmcv_new.mkdir_or_exist(out_dir)
