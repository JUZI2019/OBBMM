...
def generate_inputs_and_wrap_model(config_path,
    ...
    try:
        from mmcv_new.onnx.symbolic import register_extra_symbolics
...
def build_model_from_cfg(config_path, checkpoint_path, cfg_options=None):
    ...
    cfg = mmcv_new.Config.fromfile(config_path)
    ...
    if 'CLASSES' in checkpoint.get('meta', {}):
        ...
        from mmdet_new.datasets import DATASETS
...
def preprocess_example_input(input_config):
    ...
    Examples:
        >>> from mmdet_new.core.export import preprocess_example_input
        >>> input_config = {
    ...
    one_img = mmcv_new.imread(input_path)
    one_img = mmcv_new.imresize(one_img, input_shape[2:][::-1])
    ...
    if 'normalize_cfg' in input_config.keys():
        ...
        one_img = mmcv_new.imnormalize(one_img, mean, std, to_rgb=to_rgb)
