...
from mmdet_new.core import BitmapMasks, PolygonMasks, find_inside_bboxes
from mmdet_new.core.evaluation.bbox_overlaps import bbox_overlaps
from mmdet_new.utils import log_img_scale
...
class Resize:
    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list):
                ...
                self.img_scale = [img_scale]
            assert mmcv_new.is_list_of(self.img_scale, tuple)
    ...
    def random_select(img_scales):
        ...
        assert mmcv_new.is_list_of(img_scales, tuple)
    ...
    def random_sample(img_scales):
        ...
        assert mmcv_new.is_list_of(img_scales, tuple) and len(img_scales) == 2
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                img, scale_factor = mmcv_new.imrescale(
                    results[key],
                    results['scale'],
                    return_scale=True,
                ...
                img, w_scale, h_scale = mmcv_new.imresize(
                    results[key],
                    results['scale'],
                    return_scale=True,
                    interpolation=self.interpolation,
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = mmcv_new.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = mmcv_new.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class RandomFlip:
    ...
    def __init__(self, flip_ratio=None, direction='horizontal'):
        if isinstance(flip_ratio, list):
            assert mmcv_new.is_list_of(flip_ratio, float)
        ...
        if isinstance(direction, str):
            ...
            assert mmcv_new.is_list_of(direction, str)
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.scale_factor != 1:
                results[key] = mmcv_new.imrescale(
                    results[key],
                    self.scale_factor,
                    interpolation='nearest',
                    backend=self.backend)
...
class MixUp:
    ...
    def _mixup_transform(self, results):
        ...
        retrieve_img = mmcv_new.imresize(
        ...
        out_img = mmcv_new.imresize(out_img, (int(out_img.shape[1] * jit_factor),
