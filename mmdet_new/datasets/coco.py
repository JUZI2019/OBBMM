...
from mmdet_new.core import eval_recalls
...
class CocoDataset(CustomDataset):
    ...
    def results2json(self, results, outfile_prefix):
        ...
        if isinstance(results[0], list):
            ...
            mmcv_new.dump(json_results, result_files['bbox'])
        elif isinstance(results[0], tuple):
            json_results = self._segm2json(results)
            result_files['bbox'] = f'{outfile_prefix}.bbox.json'
            result_files['proposal'] = f'{outfile_prefix}.bbox.json'
            result_files['segm'] = f'{outfile_prefix}.segm.json'
            mmcv_new.dump(json_results[0], result_files['bbox'])
            mmcv_new.dump(json_results[1], result_files['segm'])
        elif isinstance(results[0], np.ndarray):
            json_results = self._proposal2json(results)
            result_files['proposal'] = f'{outfile_prefix}.proposal.json'
            mmcv_new.dump(json_results, result_files['proposal'])
        else:
    ...
    def evaluate_det_segm(self,
        ...
        for metric in metrics:
            ...
            try:
                predictions = mmcv_new.load(result_files[metric])
                if iou_type == 'segm':
