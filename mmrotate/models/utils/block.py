...
class C3_(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        # self.m = nn.Sequential(*(Bottleneck_(c_, c_, shortcut, g, k=((1, 1), (3, 3)), e=1.0) for _ in range(n)))
        self.m = nn.Sequential(*(Bottleneck_(c_, c_, shortcut, g, k=((1, 1), (3, 3)), e=1.0) for _ in range(n)))
...
class BottleneckCSP_(nn.Module):
    """CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks."""
...
class C3k2_(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_(self.c, self.c, 2, shortcut, g) if c3k else BottleneckCSP_(self.c, self.c, shortcut, g) for _ in range(n)
        )
...
class C3k_(C3_):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(BottleneckCSP_(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class C2CA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        # self.m = nn.Sequential(*(CABlock(self.c, attn_ratio=0.5, num_heads=self.c // 64) for _ in range(n)))
        self.m = nn.Sequential(*(CABlock(self.c, num_heads=self.c // 64) for _ in range(n)))
...
class CABlock(nn.Module):
    ...
    def __init__(self, c,  num_heads=4, shortcut=True) -> None:
        ...
        self.attn = CAFM(c, num_heads=num_heads)
        self.ffn = nn.Sequential(Conv(c, c * 2, 1), Conv(c * 2, c, 1, act=False))
        self.add = shortcut
