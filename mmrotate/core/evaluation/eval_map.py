...
def eval_rbbox_map(det_results,
    ...
    if output_csv is not None:
        for i, result in enumerate(eval_results):
            ...
            # 构建CSV文件名
            csv_filename = f"{output_csv}.csv"
            # 保存到CSV
            df.to_csv(csv_filename, index=False)
            if logger:
                logger.info(f"Saved precision and recall for class {i} to {csv_filename}")
    ...
    for i in range(num_scales):
        ...
        for j in range(num_classes):
            row_data = [
                ...
                f'{recalls[i, j]:.3f}', f'{precisions[i, j]:.3f}', f'{f1_scores[i, j]:.3f}', f'{aps[i, j]:.3f}'
            ]
