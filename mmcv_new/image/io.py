...
from mmcv_new.fileio import FileClient
from mmcv_new.utils import is_filepath, is_str
...
def imread(img_or_path: Union[np.ndarray, str, Path],
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            ...
            ``mmcv_new.use_backend()`` will be used. Default: None.
        file_client_args (dict | None): Arguments to instantiate a
            FileClient. See :class:`mmcv_new.fileio.FileClient` for details.
            Default: None.
    ...
    Examples:
        ...
        >>> img = mmcv_new.imread(img_path)
        >>> img = mmcv_new.imread(img_path, flag='color', channel_order='rgb',
        ...     backend='cv2')
        >>> img = mmcv_new.imread(img_path, flag='color', channel_order='bgr',
        ...     backend='pillow')
        >>> s3_img_path = 's3://bucket/img.jpg'
        >>> # infer the file backend by the prefix s3
        >>> img = mmcv_new.imread(s3_img_path)
        >>> # manually set the file backend petrel
        >>> img = mmcv_new.imread(s3_img_path, file_client_args={
        ...     'backend': 'petrel'})
        >>> http_img_path = 'http://path/to/img.jpg'
        >>> img = mmcv_new.imread(http_img_path)
        >>> img = mmcv_new.imread(http_img_path, file_client_args={
        ...     'backend': 'http'})
    """
...
def imfrombytes(content: bytes,
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            ...
            None, the global imread_backend specified by ``mmcv_new.use_backend()``
    ...
    Examples:
        ...
        >>>     img_buff = f.read()
        >>> img = mmcv_new.imfrombytes(img_buff)
        >>> img = mmcv_new.imfrombytes(img_buff, flag='color', channel_order='rgb')
        >>> img = mmcv_new.imfrombytes(img_buff, backend='pillow')
        >>> img = mmcv_new.imfrombytes(img_buff, backend='cv2')
    """
...
def imwrite(img: np.ndarray,
    ...
    Examples:
        ...
        >>> ret = mmcv_new.imwrite(img, '/path/to/img.jpg')
        >>> # infer the file backend by the prefix s3
        >>> ret = mmcv_new.imwrite(img, 's3://bucket/img.jpg')
        >>> # manually set the file backend petrel
        >>> ret = mmcv_new.imwrite(img, 's3://bucket/img.jpg', file_client_args={
        ...     'backend': 'petrel'})
    """
