...
from mmcv_new.runner import (HOOKS, RUNNERS, BaseRunner, EpochBasedRunner,
                         IterBasedRunner)
from mmcv_new.utils import IS_IPU_AVAILABLE
...
class IPUBaseRunner(BaseRunner):
    ...
    Args:
        ...
        options_cfg (mmcv_new.Config, dict): Options that will be used to compile
            and run the model.
        modules_to_record (mmcv_new.Config, list): Index or name of modules which
            will be recorded for output. It is necessary to specify output for
            static graph of model training or inference.
        ipu_model_cfg (mmcv_new.Config, dict): Config of model partition and
            recomputing checkpoint
        fp16_cfg (mmcv_new.Config): Config for fp16 training.
