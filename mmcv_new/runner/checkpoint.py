...
def get_torchvision_models():
    if digit_version(torchvision.__version__) < digit_version('0.13.0a0'):
        ...
        json_path = osp.join(mmcv_new.__path__[0],
                             'model_zoo/torchvision_0.12.json')
        model_urls = mmcv_new.load(json_path)
...
def get_external_models():
    ...
    default_json_path = osp.join(mmcv_new.__path__[0], 'model_zoo/open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(mmcv_new.__path__[0], 'model_zoo/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(mmcv_new.__path__[0],
...
def load_from_ceph(filename: str,
    ...
    .. warning::
        :class:`mmcv_new.fileio.file_client.CephBackend` will be deprecated,
        please use :class:`mmcv_new.fileio.file_client.PetrelBackend` instead.
...
def save_checkpoint(model: torch.nn.Module,
    ...
    meta.update(mmcv_version=mmcv_new.__version__, time=time.asctime())
